...
public class MainWorldGen implements IWorldGenerator {
	...
	private void generateInOverworld(World world, Random random, int x, int z) {
		...
		for (int i=0; i<5; i++) {
			...
			(new WorldGenMinable(BlockHandler.blockBloodDiamondOre, 4)).generate(world, random, chunkX, chunkY, chunkZ);
		}
		...
		for (int i=0; i<80; i++) {
			...
			(new WorldGenMinable(BlockHandler.blockXPStone, 20)).generate(world, random, chunkX, chunkY, chunkZ);
		}
